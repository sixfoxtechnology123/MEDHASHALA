...
const QuestionBank = () => {
  ...
  const [filters, setFilters] = useState({ examMasterId: "", examCategoryId: "", subjectId: "", status: "" });
  ...
  }, [filters.examMasterId, filters.examCategoryId, filters.subjectId, filters.status, pagination.page, pagination.limit]);
  ...
  const openAddModal = async () => {
    try {
      ...
      setEditId("");
      setDraftQuestions([]);
      setBaseForm({ examMasterId: "", examCategoryId: "", subjectId: "" });
  ...
  const handleEdit = (row) => {
    setEditId(row._id);
    setDraftQuestions([]);
    setBaseForm({ examMasterId: row.examMasterId || "", examCategoryId: row.examCategoryId || "", subjectId: row.subjectId || "" });
    setQuestionForm({
      ...
      <header className="p-2 space-y-2">
        <div className="bg-white px-4 py-2 rounded-[24px] border border-slate-200 shadow-sm flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <select value={filters.examMasterId} onChange={(e) => { setFilters((prev) => ({ ...prev, examMasterId: e.target.value, examCategoryId: "", subjectId: "" })); setPagination((prev) => ({ ...prev, page: 1 })); }} className="w-full p-2 border border-slate-200 rounded-xl text-sm font-normal">
              <option value="">All Exams</option>
              {exams.map((ex) => <option key={ex.examCode} value={ex.examCode}>{ex.examName}</option>)}
            </select>
            <select value={filters.examCategoryId} onChange={(e) => { setFilters((prev) => ({ ...prev, examCategoryId: e.target.value, subjectId: "" })); setPagination((prev) => ({ ...prev, page: 1 })); }} className="w-full p-2 border border-slate-200 rounded-xl text-sm font-normal">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? <tr><td colSpan="8" className="p-20 text-center text-slate-300 font-semibold">Loading...</td></tr> : rows.length > 0 ? rows.map((row) => (
                <tr key={row._id} className="hover:bg-blue-50/30">
                  <td className="p-2 text-blue-600 text-sm font-semibold">{row.questionBankId}</td><td className="p-2 text-xs font-normal">{row.examName}</td><td className="p-2 text-xs font-normal">{row.categoryName}</td><td className="p-2 text-xs font-normal">{row.subjectName}</td><td className="p-2 text-xs font-normal">{row.marks}</td><td className="p-2 text-xs font-normal">{row.negativeMarks}</td><td className="p-2 text-xs font-normal">{row.status}</td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <form onSubmit={handleSubmit} className="relative bg-white w-full max-w-6xl p-8 rounded-[30px] shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div><label className="text-sm font-normal block mb-1">Exam</label><select required value={baseForm.examMasterId} onChange={(e) => setBaseForm({ ...baseForm, examMasterId: e.target.value, examCategoryId: "", subjectId: "" })} className="w-full p-3 border border-slate-200 rounded-xl text-sm font-normal"><option value="">-- Select Exam --</option>{exams.map((ex) => <option key={ex.examCode} value={ex.examCode}>{ex.examName}</option>)}</select></div>
              <div><label className="text-sm font-normal block mb-1">Category</label><select required value={baseForm.examCategoryId} onChange={(e) => setBaseForm({ ...baseForm, examCategoryId: e.target.value, subjectId: "" })} className="w-full p-3 border border-slate-200 rounded-xl text-sm font-normal"><option value="">-- Select Category --</option>{modalFilteredCategories.map((cat) => <option key={cat.catId} value={cat.catId}>{cat.catName}</option>)}</select></div>
